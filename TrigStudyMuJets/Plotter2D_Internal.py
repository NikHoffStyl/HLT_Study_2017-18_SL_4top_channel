...
def fitInfo(fit, fitName, args, writeOpt):
    ...
    """
    filename = pwd + "/fitInfo/Era" + args.inputLFN + ".txt"
    if not os.path.exists(os.path.dirname(filename)):
        try:
            os.makedirs(os.path.dirname(filename))
        except OSError as exc:
...
def cmsPlotString(args):
    ...
    """
    if args == "17B":
        legStr = "#bf{CMS Internal} Run2017B   4.82 fb^{-1} (13TeV)"
    elif args == "17C":
        legStr = "#bf{CMS Internal} Run2017C  9.66 fb^{-1} (13TeV)"
    elif args == "17D":
        legStr = "#bf{CMS Internal} Run2017D  4.25 fb^{-1} (13TeV)"
    elif args == "17E":
        legStr = "#bf{CMS Internal} Run2017E  9.28 fb^{-1} (13TeV)"
    elif args == "17F":
        legStr = "#bf{CMS Internal} Run2017F  13.52 fb^{-1} (13TeV)"
    elif args == "17DEF":
        legStr = "#bf{CMS Internal} Run2017D-F  27.05 fb^{-1} (13TeV)"
    elif args == "17CDEF":
        legStr = "#bf{CMS Internal} Run2017C-F  36.71 fb^{-1} (13TeV)"
    elif args == "all":
        legStr = "#bf{CMS Internal} All Run2017  41.53 fb^{-1} (13TeV)"
    elif args == "18A":
        legStr = "#bf{CMS Internal} Run2018A  14.00 fb^{-1} (13TeV)"
    elif args == "18B":
        legStr = "#bf{CMS Internal} Run2018B  7.10 fb^{-1} (13TeV)"
    elif args == "18C":
        legStr = "#bf{CMS Internal} Run2018C   6.94 fb^{-1} (13TeV)"
    elif args == "18D":
        legStr = "#bf{CMS Internal} Run2018D   31.93 fb^{-1} (13TeV)"
    elif args == "18AB":
        legStr = "#bf{CMS Internal} Run2018A-B  21.10 fb^{-1} (13TeV)"
    elif args == "18CD":
        legStr = "#bf{CMS Internal} Run2018C-D  38.87 fb^{-1} (13TeV)"
    else:
        legStr = "#bf{CMS Internal}"
...
def cmsPlotStringBase(args):
    ...
    """
    if args == "17B":
        legStr = "#bf{CMS Internal} Run2017B Baseline 4.82 fb^{-1} (13TeV)"
    elif args == "17C":
        legStr = "#bf{CMS Internal} Run2017C Baseline 9.66 fb^{-1} (13TeV)"
    elif args == "17D":
        legStr = "#bf{CMS Internal} Run2017D Baseline 4.25 fb^{-1} (13TeV)"
    elif args == "17E":
        legStr = "#bf{CMS Internal} Run2017E Baseline 9.28 fb^{-1} (13TeV)"
    elif args == "17F":
        legStr = "#bf{CMS Internal} Run2017F Baseline 13.52 fb^{-1} (13TeV)"
    elif args == "17DEF":
        legStr = "#bf{CMS Internal} Run2017D-F Baseline 27.05 fb^{-1} (13TeV)"
    elif args == "17CDEF":
        legStr = "#bf{CMS Internal} Run2017C-F Baseline 36.71 fb^{-1} (13TeV)"
    elif args == "all":
        legStr = "#bf{CMS Internal} All Run2017 Baseline 41.53 fb^{-1} (13TeV)"
    elif args == "18A":
        legStr = "#bf{CMS Internal} Run2018A Baseline 14.00 fb^{-1} (13TeV)"
    elif args == "18B":
        legStr = "#bf{CMS Internal} Run2018B Baseline 7.10 fb^{-1} (13TeV)"
    elif args == "18C":
        legStr = "#bf{CMS Internal} Run2018C Baseline 6.94 fb^{-1} (13TeV)"
    elif args == "18D":
        legStr = "#bf{CMS Internal} Run2018D Baseline 31.93 fb^{-1} (13TeV)"
    elif args == "18AB":
        legStr = "#bf{CMS Internal} Run2018A-B Baseline 21.10 fb^{-1} (13TeV)"
    elif args == "18CD":
        legStr = "#bf{CMS Internal} Run2018C-D Baseline 38.87 fb^{-1} (13TeV)"
    else:
        legStr = "#bf{CMS Internal} Baseline"
...
def findTrigRatio(h1, title, newDir):
    ...
    if bool(h1) == False:
        ...
        if "18CD" in title or "18ABtt" in title: hltTypes = ["Muon", "Electron", "Jet2", "El_CROSS_Jets", "El_OR_Jets2",
                                                             "Mu_OR_Jets2"]
        channels = ["Mu_", "El_"]
            ...
            for xAxis in xAxes:
                ...
                for hlt in hltTypes:
                    ...
                    if h1[histNameNum].GetNbinsX() != h1[histNameDen].GetNbinsX():
                        print("\n ERROR different hists size: %s  %d  %d " % (
                        histNameNum, h_TH1DOut[histNameNum].GetNbinsX(), h1[histNameDen].GetNbinsX()))
...
def scaleFactor(h1, h2, title, era, newDir):
    ...
    Args:
        ...
        era: run period
        newDir (TDirectory): root directory
    Returns:
    ...
    if bool(h1) == False or bool(h2) == False:
        print(era + " SF plots removed: " + title)
...
def main():
    ...
    parser.add_argument("-i", "--imageName", default="vInt_HistFiles/plotImages/", help="Set directory to save images")
    parser.add_argument("-pdf", "--pdfName", default="vInt_HistFiles/plotPDFs/", help="Set directory to save pdfs")
    ...
    # s_dataSMu_B, hNamesMu_B = scaleFactor(tr_dataSMu_B, tr_mcTTToSemiLep_B, "Single Muon Data 17B", args.inputLFN,
    #                                       sfDIR)
    # s_dataSEl_B, hNamesEl_B = scaleFactor(tr_dataSEl_B, tr_mcTTToSemiLep_B, "Single Electron Data 17B", args.inputLFN,
    #                                       sfDIR)
    s_dataMuHTs17B, hNamesHTMu_B = scaleFactor(tr_dataMuHTs17B, tr_mcTTToSemiLep_B, "All Data 17B", args.inputLFN,
                                               sfDIR)
    s_dataElHTs17B, hNamesHTEl_B = scaleFactor(tr_dataElHTs17B, tr_mcTTToSemiLep_B, "All Data 17B", args.inputLFN,
                                               sfDIR)
    ...
    # s_dataSEl_C, hNamesEl_C = scaleFactor(tr_dataSEl_C, tr_mcTTToSemiLep_C, "Single Electron Data", args.inputLFN,
    #                                       sfDIR)
    s_dataMuHTs17C, hNamesHTMu_C = scaleFactor(tr_dataMuHTs17C, tr_mcTTToSemiLep_C, "All Data", args.inputLFN, sfDIR)
    s_dataElHTs17C, hNamesHTEl_C = scaleFactor(tr_dataElHTs17C, tr_mcTTToSemiLep_C, "All Data 17C", args.inputLFN,
                                               sfDIR)
    ...
    s_dataMuHTs17DEF, hNamesHTMu_DEF = scaleFactor(tr_dataMuHTs17DEF, tr_mcTTToSemiLepDEF, "All Data", args.inputLFN,
                                                   sfDIR)
    s_dataElHTs17DEF, hNamesHTEl_DEF = scaleFactor(tr_dataElHTs17DEF, tr_mcTTToSemiLepDEF, "All Data", args.inputLFN,
                                                   sfDIR)

    s_dataMuHTs18AB, hNamesHTMu_AB = scaleFactor(tr_dataMuHTs18AB, tr_mcTTToSemiLep_18A, "All Data 18AB", args.inputLFN,
                                                 sfDIR)
    s_dataElHTs18AB, hNamesHTEl_AB = scaleFactor(tr_dataElHTs18AB, tr_mcTTToSemiLep_18A, "All Data 18AB", args.inputLFN,
                                                 sfDIR)
    s_dataMuHTs18CD, hNamesHTMu_CD = scaleFactor(tr_dataMuHTs18CD, tr_mcTTToSemiLep_18A, "All Data 18CD", args.inputLFN,
                                                 sfDIR)
    s_dataElHTs18CD, hNamesHTEl_CD = scaleFactor(tr_dataElHTs18CD, tr_mcTTToSemiLep_18A, "All Data 18CD", args.inputLFN,
                                                 sfDIR)
    ...
    if args.inputLFN == "17B":
        for hName in h_mcTTJets17B:
            ...
            if args.outputName == "ttsemi":
                h_HLTcompare[hName] = h_mcTTToSemiLeps17B[hName]
            elif args.outputName == "ttjets":
                h_HLTcompare[hName] = h_mcTTJets17B[hName]
        ...
        for hName in h_dataHTMHTs17B:
            ...
            if args.outputName == "smu":
                h_HLTcompare[hName] = h_dataSMus17B[hName]
            elif args.outputName == "sel":
                h_HLTcompare[hName] = h_dataSEls17B[hName]
            elif args.outputName == "ht":
                h_HLTcompare[hName] = h_dataHTMHTs17B[hName]
            elif args.outputName == "htel":
                h_HLTcompare[hName] = h_dataElHTs17B[hName]
            elif args.outputName == "htmu":
                h_HLTcompare[hName] = h_dataMuHTs17B[hName]
        ...
        for hName in h_dataHTMHTs17C:
            ...
            # h_dataSEls[hName].Write(hName)
            if args.outputName == "ttsemi":
                h_HLTcompare[hName] = h_mcTTToSemiLeps17C[hName]
            elif args.outputName == "ttjets":
                h_HLTcompare[hName] = h_mcTTJets17C[hName]
            elif args.outputName == "smu":
                h_HLTcompare[hName] = h_dataSMus17C[hName]
            elif args.outputName == "sel":
                h_HLTcompare[hName] = h_dataSEls17C[hName]
            elif args.outputName == "ht":
                h_HLTcompare[hName] = h_dataHTMHTs17C[hName]
            elif args.outputName == "htel":
                h_HLTcompare[hName] = h_dataElHTs17C[hName]
            elif args.outputName == "htmu":
                h_HLTcompare[hName] = h_dataMuHTs17C[hName]
        ...
        for hName in h_dataHTMHTs17DEF:
            ...
            if args.outputName == "ttsemi":
                h_HLTcompare[hName] = h_mcTTToSemiLeps[hName]
            elif args.outputName == "ttjets":
                h_HLTcompare[hName] = h_mcTTJets17D[hName]
            elif args.outputName == "smu":
                h_HLTcompare[hName] = h_dataSMus17DEF[hName]
            elif args.outputName == "sel":
                h_HLTcompare[hName] = h_dataSEls17DEF[hName]
            elif args.outputName == "ht":
                h_HLTcompare[hName] = h_dataHTMHTs17DEF[hName]
            elif args.outputName == "htel":
                h_HLTcompare[hName] = h_dataElHTs17DEF[hName]
            elif args.outputName == "htmu":
                h_HLTcompare[hName] = h_dataMuHTs17DEF[hName]
    elif args.inputLFN == "18AB":
        # print(" ")
        for hName in h_dataHTMHTs18AB:
            if hName[-3:] == "Jet" or hName[-7:] == "OR_Jets" or hName[-6:] == "no-HLT":
                hName2 = hName + "2"
            else:
                hName2 = hName
            print(hName2)
            ...
            if args.outputName == "ttsemi":
                h_HLTcompare[hName2] = h_mcTTToSemiLeps[hName2]
            elif args.outputName == "ttjets":
                h_HLTcompare[hName2] = h_mcTTJets18A[hName2]
            elif args.outputName == "htel":
                h_HLTcompare[hName] = h_dataElHTs18AB[hName]
            elif args.outputName == "htmu":
                h_HLTcompare[hName] = h_dataMuHTs18AB[hName]
        ...
        for hName in h_dataHTMHTs18CD:
            ...
            if args.outputName == "ttsemi":
                ...
                h_HLTcompare[hName2] = h_dataElHTs18CD[hName2]
            elif args.outputName == "htmu":
                h_HLTcompare[hName2] = h_dataMuHTs18CD[hName2]
    ...
    t1.SetTextSize(0.03)
    if args.outputName == "ttsemi":
        t1.AddText("#bf{CMS Internal}                                #sigma(t#bar{t}) = 374 pb (13TeV)")
    elif args.outputName == "ttjets":
        t1.AddText("#bf{CMS Internal}                                #sigma(t#bar{t}) = 831 pb (13TeV)")
    elif args.outputName == "smu":
        t1.AddText("#bf{CMS Internal}    " + runName + "    " + inLumi + " fb^{-1} (13TeV)")
    elif args.outputName == "sel":
        t1.AddText("#bf{CMS Internal}    " + runName + "    " + inLumi + " fb^{-1} (13TeV)")
    elif args.outputName == "htmu":
        t1.AddText("#bf{CMS Internal}    " + runName + "    " + inLumi + " fb^{-1} (13TeV)")
        ...
        t1.AddText("#bf{CMS Internal}                           #sigma(t#bar{t}t#bar{t}) = 9.2 fb (13TeV)")
    ...
    #        t2.Clear()
    #        t2.AddText("#bf{CMS Internal} " + runName + " Baseline  #sigma(t#bar{t}) = 831 pb (13TeV)")
    #        t2.Draw("same")
    ...
    for hn, hName in enumerate(hNames):
        ...
        h_mcTTToSemiLeps[hName2].Draw("TEXTE COLZ")
        h_mcTTToSemiLeps[hName2].GetZaxis().SetTitleOffset(1.8)
        t2.Clear()
        # t2.AddText("#bf{CMS Internal}  " + runName + " Baseline #sigma(t#bar{t}) = 831 pb (13TeV)")
        t2.AddText("#bf{CMS Internal}  " + runName + "          #sigma(t#bar{t}) = 831 pb (13TeV)")
        t2.Draw("same")
        triggerCanvas.Print(args.imageName + "events/ttMC_{0}events{1}.png".format(hName2, era), "png")
        if "nBJet_" in hName:
            h_mcTTToSemiLeps[hName2].GetXaxis().SetRangeUser(0, 7)
        elif "nJet_" in hName:
            h_mcTTToSemiLeps[hName2].GetXaxis().SetRangeUser(6, 14)
        elif "lepEta_" in hName:
            h_mcTTToSemiLeps[hName2].GetXaxis().SetRangeUser(-4, 4)
        elif "lepPhi_" in hName:
            h_mcTTToSemiLeps[hName2].GetXaxis().SetRangeUser(-4, 4)
        elif "HT_" in hName:
            h_mcTTToSemiLeps[hName2].GetXaxis().SetRangeUser(0, 3000)
        h_mcTTToSemiLeps[hName2].SetTitle("t#bar{t} pair MC (%d)" % histEntries)
        ...
        if hName.find("Mu") != -1:
            ...
            print(">>>>>>>  {0}".format(hName))
            cv0[n_count] = triggerCanvas.cd(1)
            h_dataMuHTs[hName].Draw('TEXTE COLZ')
            h_dataMuHTs[hName].GetZaxis().SetTitleOffset(1.8)
            t2String = legString.replace("Run", "#mu-chan. Run")
            t2.Clear()
            t2.AddText(t2String)
            ...
            print(">>>>>>>  {0}".format(hName))
            cv0[n_count] = triggerCanvas.cd(1)
            h_dataElHTs[hName].Draw('TEXTE COLZ')
            h_dataElHTs[hName].GetZaxis().SetTitleOffset(1.8)
            t2String = legString.replace("Run", "e-chan. Run")
            t2.Clear()
            t2.AddText(t2String)
        ...
        if "18" in era:
            if hName[-3:] == "Jet" or hName[-7:] == "OR_Jets":
                hName2 = hName + "2"
            else:
                hName2 = hName
        else:
            hName2 = hName
        tr_mcTTToSemiLep[hName2].Draw('TEXTE COLZ')
        ...
        if "Mu" in hName:
            tr_dataMuHTs[hName].Draw('TEXTE COLZ')
            ...
            tr_dataElHTs[hName].Draw('TEXTE COLZ')
            t2String = legString.replace("Run", "e-chan. Run")
            t2.Clear()
            t2.AddText(t2String)
        t2.Draw("same")
        ...
        t3.SetTextSize(0.03)
        t3.AddText("#bf{CMS Internal}")
        if era == "18CD":
            if "Jet2" in hName or "OR_Jets2" in hName:
                hName3 = hName.replace("2", "")
            else:
                hName3 = hName
        else:
            hName3 = hName
        if "Mu" in hName:
            if "nBJet_" in hName3:
                s_dataMuHTs17B[hName3].GetXaxis().SetRangeUser(0, 7)
            elif "nJet_" in hName3:
                s_dataMuHTs17B[hName3].GetXaxis().SetRangeUser(6, 14)
            elif "lepEta_" in hName3:
                s_dataMuHTs17B[hName3].GetXaxis().SetRangeUser(-4, 4)
            elif "lepPhi_" in hName3:
                s_dataMuHTs17B[hName3].GetXaxis().SetRangeUser(-4, 4)
            elif "HT_" in hName3:
                s_dataMuHTs17B[hName3].GetXaxis().SetRangeUser(0, 3000)
            s_dataMuHTs17B[hName3].Draw('TEXTE COLZ')
            ...
            s_dataMuHTs17C[hName3].Draw('TEXTE COLZ')
            ...
            s_dataMuHTs17DEF[hName3].Draw('TEXTE COLZ')
            ...
            s_dataMuHTs18AB[hName3].Draw('TEXTE COLZ')
            ...
            if hName[-3:] == "Jet" or hName[-7:] == "OR_Jets":
                ...
                hName2 = hName
            s_dataMuHTs18CD[hName2].Draw('TEXTE COLZ')
            ...
            if ("HT" in hName) and "OR" in hName:
                with open('SF_17B_el_file.csv', mode='w') as sf_file:
                    ...
                    for binX in range(1, nXBins + 1):
                        ...
                        for binY in range(1, nYBins + 1):
                            ...
                            if bin_sf < 0.00001:
                                bin_sf = 1
                                bin_error = 0.001
                            write_list = [hName3, bin_lowEdgeX, bin_upEdgeX, bin_lowEdgeY, bin_upEdgeY, bin_sf,
                                          bin_error]
                            sf_writer.writerow(write_list)
                with open('SF_17C_el_file.csv', mode='w') as sf_file:
                    sf_writer = csv.writer(sf_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_NONNUMERIC)
                    ...
                    for binX in range(1, nXBins + 1):
                        ...
                        for binY in range(1, nYBins + 1):
                            ...
                            write_list = [hName3, bin_lowEdgeX, bin_upEdgeX, bin_lowEdgeY, bin_upEdgeY, bin_sf,
                                          bin_error]
                            sf_writer.writerow(write_list)
                with open('SF_17DEF_el_file.csv', mode='w') as sf_file:
                    sf_writer = csv.writer(sf_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_NONNUMERIC)
                    ...
                    for binX in range(1, nXBins + 1):
                        ...
                        for binY in range(1, nYBins + 1):
                            ...
                            write_list = [hName3, bin_lowEdgeX, bin_upEdgeX, bin_lowEdgeY, bin_upEdgeY, bin_sf,
                                          bin_error]
                            sf_writer.writerow(write_list)
                with open('SF_18AB_el_file.csv', mode='w') as sf_file:
                    sf_writer = csv.writer(sf_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_NONNUMERIC)
                    ...
                    for binX in range(1, nXBins + 1):
                        ...
                        for binY in range(1, nYBins + 1):
                            ...
                            write_list = [hName3, bin_lowEdgeX, bin_upEdgeX, bin_lowEdgeY, bin_upEdgeY, bin_sf,
                                          bin_error]
                            sf_writer.writerow(write_list)
                with open('SF_18CD_el_file.csv', mode='w') as sf_file:
                    sf_writer = csv.writer(sf_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_NONNUMERIC)
                    ...
                    for binX in range(1, nXBins + 1):
                        ...
                        for binY in range(1, nYBins + 1):
                            ...
                            if bin_sf < 0.00001:
                                bin_sf = 1
                                bin_error = 0.001
                            write_list = [hName2, bin_lowEdgeX, bin_upEdgeX, bin_lowEdgeY, bin_upEdgeY, bin_sf,
                                          bin_error]
                            sf_writer.writerow(write_list)

        elif "El" in hName:
            if "nBJet_" in hName3:
                s_dataElHTs17B[hName3].GetXaxis().SetRangeUser(0, 7)
            elif "nJet_" in hName3:
                s_dataElHTs17B[hName3].GetXaxis().SetRangeUser(6, 14)
            elif "lepEta_" in hName3:
                s_dataElHTs17B[hName3].GetXaxis().SetRangeUser(-4, 4)
            elif "lepPhi_" in hName3:
                s_dataElHTs17B[hName3].GetXaxis().SetRangeUser(-4, 4)
            elif "HT_" in hName3:
                s_dataElHTs17B[hName3].GetXaxis().SetRangeUser(0, 3000)
            s_dataElHTs17B[hName3].Draw('TEXTE COLZ')
            ...
            s_dataElHTs17C[hName3].Draw('TEXTE COLZ')
            ...
            s_dataElHTs17DEF[hName3].Draw('TEXTE COLZ')
            ...
            s_dataElHTs18AB[hName3].Draw('TEXTE COLZ')
            ...
            if hName[-3:] == "Jet" or hName[-7:] == "OR_Jets":
                ...
                hName2 = hName
            s_dataElHTs18CD[hName2].Draw('TEXTE COLZ')
            ...
            if ("HT" in hName) and "OR" in hName:
                with open('SF_17B_mu_file.csv', mode='w') as sf_file:
                    sf_writer = csv.writer(sf_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_NONNUMERIC)
                    nXBins = s_dataElHTs17B[hName3].GetNbinsX()
                    nYBins = s_dataElHTs17B[hName3].GetNbinsY()
                    for binX in range(1, nXBins + 1):
                        ...
                        for binY in range(1, nYBins + 1):
                            ...
                            write_list = [hName3, bin_lowEdgeX, bin_upEdgeX, bin_lowEdgeY, bin_upEdgeY, bin_sf,
                                          bin_error]
                            sf_writer.writerow(write_list)
                with open('SF_17C_mu_file.csv', mode='w') as sf_file:
                    sf_writer = csv.writer(sf_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_NONNUMERIC)
                    nXBins = s_dataElHTs17C[hName3].GetNbinsX()
                    nYBins = s_dataElHTs17C[hName3].GetNbinsY()
                    for binX in range(1, nXBins + 1):
                        ...
                        for binY in range(1, nYBins + 1):
                            ...
                            write_list = [hName3, bin_lowEdgeX, bin_upEdgeX, bin_lowEdgeY, bin_upEdgeY, bin_sf,
                                          bin_error]
                            sf_writer.writerow(write_list)
                with open('SF_17DEF_mu_file.csv', mode='w') as sf_file:
                    sf_writer = csv.writer(sf_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_NONNUMERIC)
                    nXBins = s_dataElHTs17DEF[hName3].GetNbinsX()
                    nYBins = s_dataElHTs17DEF[hName3].GetNbinsY()
                    for binX in range(1, nXBins + 1):
                        ...
                        for binY in range(1, nYBins + 1):
                            ...
                            write_list = [hName3, bin_lowEdgeX, bin_upEdgeX, bin_lowEdgeY, bin_upEdgeY, bin_sf,
                                          bin_error]
                            sf_writer.writerow(write_list)
                with open('SF_18AB_mu_file.csv', mode='w') as sf_file:
                    sf_writer = csv.writer(sf_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_NONNUMERIC)
                    nXBins = s_dataElHTs18AB[hName3].GetNbinsX()
                    nYBins = s_dataElHTs18AB[hName3].GetNbinsY()
                    for binX in range(1, nXBins + 1):
                        ...
                        for binY in range(1, nYBins + 1):
                            ...
                            write_list = [hName3, bin_lowEdgeX, bin_upEdgeX, bin_lowEdgeY, bin_upEdgeY, bin_sf,
                                          bin_error]
                            sf_writer.writerow(write_list)
                with open('SF_18CD_mu_file.csv', mode='w') as sf_file:
                    sf_writer = csv.writer(sf_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_NONNUMERIC)
                    nXBins = s_dataElHTs18CD[hName2].GetNbinsX()
                    nYBins = s_dataElHTs18CD[hName2].GetNbinsY()
                    for binX in range(1, nXBins + 1):
