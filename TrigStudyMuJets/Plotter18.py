...
def main():
    ...
    for hn, hName in enumerate(hNames):
        ...
        for i in range(0, 18):
            binWidth = h_mcTTJets[hName2].GetXaxis().GetBinWidth(i)
            binContent = h_mcTTJets[hName2].GetBinContent(i)
            newBinContent = round(binContent / binWidth)
            h_mcTTJets[hName].SetBinContent(i, newBinContent)
            binContent = h_dataHTMHTs[hName2].GetBinContent(i)
            newBinContent = round(binContent / binWidth)
            h_dataHTMHTs[hName].SetBinContent(i, newBinContent)
            binContent = h_dataSMus[hName2].GetBinContent(i)
            newBinContent = round(binContent / binWidth)
            h_dataSMus[hName].SetBinContent(i, newBinContent)
            binContent = h_dataSEls[hName2].GetBinContent(i)
            newBinContent = round(binContent / binWidth)
            h_dataSEls[hName].SetBinContent(i, newBinContent)
        maxYs = getMaxY([h_mcTTJets[hName2], h_dataSMus[hName], h_dataHTMHTs[hName], h_dataSEls[hName]])
        h_mcTTJets[hName2].SetMaximum(maxYs + 100)
        h_mcTTJets[hName2].SetLineColor(1)
        h_mcTTJets[hName2].Draw("hist")
        histEntries = h_mcTTJets[hName2].GetEntries()
        h_mcTTJets[hName2].SetTitle("t#bar{t} pair MC (%d)" % histEntries)
        newName = h_mcTTJets[hName2].GetName()
        print(newName)
        legend3[n_count].AddEntry(h_mcTTJets[hName2], "t#bar{t} pair MC (%d)" % histEntries, 'l')
        ...
        tr2_mcTTJet[hName2].SetLineColor(1)
        tr2_mcTTJet[hName2].Draw()
        newName = tr2_mcTTJet[hName2].GetName()
        legend4[n_count].AddEntry(newName, "t#bar{t} pair MC", 'lep')
        cv1[n_count].Update()
        graph1 = tr2_mcTTJet[hName2].GetPaintedGraph()
        graph1.SetMinimum(0)
        graph1.SetMaximum(1.2)
        ...
        h_mcTTJets[hName2].Write(mcName)
