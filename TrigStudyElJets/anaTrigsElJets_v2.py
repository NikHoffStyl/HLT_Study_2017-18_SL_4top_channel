...
class TriggerStudy(Module):
    ...
    def __init__(self, writeHistFile=True, eventLimit=-1, trigLst=None, era=None):
        ...
        self.h_electronPhi = {}
        self.h_electronMap = {}
        self.h_electronIsolation = {}
        self.h_electronIsoPt = {}
        ...
        # self.h_electronGenPartFlav = ROOT.TH1D('h_electronGenPartFlav', 'genPartFlav_afterCriteria; GenPartFlav; '
        #                                                         'Number of events', 16, 0, 16)
        # self.h_electronGenPartIdx = ROOT.TH1D('h_electronGenPartIdx', 'genPartIdx_afterCriteria; GenPartIdx; '
        #                                                       'Number of events', 182, -2, 180)
        self.h_electronRelIso04_all = ROOT.TH1D('h_electronRelIso04_all', 'electronRelIso04_all;electronRelIso04_all;Number of Events',
                                            50, 0, 0.2)
    ...
    def beginJob(self, histFile=None, histDirName=None):
        ...
        # ELECTRON HISTOGRAMS #
        ##################
        self.h_electronPt['no_trigger'] = ROOT.TH1D('h_electronPt_notrigger', 'no trigger ;Electron P_{T} (GeV/c);Number of Events'
                                                                      ' per 1 GeV/c', 300, 0, 300)
        self.h_electronPt['prompt'] = ROOT.TH1D('h_electronPt_prompt', 'prompt electrons ;Electron P_{T} (GeV/c);Number of '
                                                               'Events per 1 GeV/c', 300, 0, 300)
        self.h_electronPt['non-prompt'] = ROOT.TH1D('h_electronPt_non-prompt',
                                                'non-prompt electrons ;Electron P_{T} (GeV/c);Number of Events '
                                                'per 1 GeV/c', 300, 0, 300)
        self.h_electronEta['no_trigger'] = ROOT.TH1D('h_electronEta_notrigger', 'no trigger ;Electron #eta;Number of Events per '
                                                                        '#delta#eta = 0.046', 300, -6, 8)
        self.h_electronPhi['no_trigger'] = ROOT.TH1D('h_electronPhi_notrigger', 'no trigger ;Electron #phi;Number of Events per '
                                                                        '#delta#phi = 0.046', 300, -6, 8)
        self.h_electronIsolation['no_trigger'] = ROOT.TH1D('h_electronIsolation_notrigger', 'no trigger ;Electron miniPFRelIso_all;'
                                                                                    'Number of Events', 30, 0, 0.17)
        self.h_electronIsoPt['no_trigger'] = ROOT.TH2F('h_electronIsoPt_notrigger', 'no trigger ;Electron P_{T} (GeV/c);'
                                                                            'Electron miniPFRelIso_all',
                                                   300, 0, 300, 30, 0, 0.17)
        self.h_electronMap['no_trigger'] = ROOT.TH2F('h_electronMap_notrigger', 'no trigger;Electron #eta;Electron #phi;',
                                                 150, -6, 6, 160, -3.2, 3.2)
        ##################
        # MET HISTOGRAMS #
        ##################
        ...
        self.addObject(self.h_electronPt['no_trigger'])
        self.addObject(self.h_electronPt['prompt'])
        self.addObject(self.h_electronPt['non-prompt'])
        # self.addObject(self.h_electronGenPartFlav)
        # self.addObject(self.h_electronGenPartIdx)
        self.addObject(self.h_electronRelIso04_all)
        self.addObject(self.h_electronEta['no_trigger'])
        self.addObject(self.h_electronPhi['no_trigger'])
        self.addObject(self.h_electronIsolation['no_trigger'])
        self.addObject(self.h_electronIsoPt['no_trigger'])
        self.addObject(self.h_electronMap['no_trigger'])
        ...
        for key in self.trigLst:
            ...
            for trgPath in self.trigLst[key]:
                ...
                self.h_electronPt[trgPath] = ROOT.TH1D('h_electronPt_' + trgPath, trgPath + ';Electron P_{T} (GeV/c);Number of '
                                                                                    'Events per 1 GeV/c', 300, 0, 300)
                self.addObject(self.h_electronPt[trgPath])
                self.h_electronEta[trgPath] = ROOT.TH1D('h_electronEta_' + trgPath, trgPath + ';Electron #eta;Number of Events per'
                                                                                      ' #delta#eta = 0.046', 300, -6, 8)
                self.addObject(self.h_electronEta[trgPath])
                self.h_electronPhi[trgPath] = ROOT.TH1D('h_electronPhi_' + trgPath, trgPath + ';Electron #phi;Number of Events per'
                                                                                      ' #delta#phi = 0.046', 300, -6, 8)
                self.addObject(self.h_electronPhi[trgPath])
                self.h_electronIsolation[trgPath] = ROOT.TH1D('h_electronIsolation_' + trgPath,
                                                          trgPath + ';Electron miniPFRelIso_all;Number of Events', 30, 0, 0.17)
                self.addObject(self.h_electronIsolation[trgPath])
                self.h_electronIsoPt[trgPath] = ROOT.TH2F('h_electronIsoPt_' + trgPath, trgPath + ';Electron P_{T} (GeV/c);'
                                                                                          'Electron miniPFRelIso_all',
                                                      300, 0, 300, 30, 0, 0.17)
                self.addObject(self.h_electronIsoPt[trgPath])
                self.h_electronMap[trgPath] = ROOT.TH2F('h_electronMap_' + trgPath, trgPath + ';Electron #eta;Electron #phi',
                                                    150, -3, 3, 160, -3.2, 3.2)
                self.addObject(self.h_electronMap[trgPath])  # - Draw ith CONTZ COLZPOL COLZ1 ARR E
    ...
    def electronCriteria(self, electrons):
        ...
        Returns:
            tuple: tuple containing
                nElsPass (int): number of electrons
                ElsPassIdx (int): index of electron that passed
        """
        nElsPass = 0
        ElsPassIdx = 0
    ...
    def analyze(self, event):
        ...
        ##############################
        if nJetPass > 5 and nMuonPass == 0 and nBtagPass > 1 and nElPass == 1:
            for nj, jet in enumerate(jets):
            ...
            if jetHt["notrig"] > 500:
                for nm, electron in enumerate(electrons):
                    ...
                    self.h_electronPhi['no_trigger'].Fill(electron.phi)
                    self.h_electronMap['no_trigger'].Fill(electron.eta, electron.phi)
                    self.h_electronIsolation['no_trigger'].Fill(electron.miniPFRelIso_all)
                    self.h_electronIsoPt['no_trigger'].Fill(electron.pt, electron.miniPFRelIso_all)
                    self.h_electronPt['no_trigger'].Fill(electron.pt)
                    for key in self.trigLst:
                        if not key.find("Mu") == -1: continue
                        for tg in self.trigLst[key]:
                            if trigPath[tg]:
                                self.h_electronPt[tg].Fill(electron.pt)
                                self.h_electronEta[tg].Fill(electron.eta)
                                self.h_electronPhi[tg].Fill(electron.phi)
                                self.h_electronMap[tg].Fill(electron.eta, electron.phi)
                                self.h_electronIsolation[tg].Fill(electron.miniPFRelIso_all)
                                self.h_electronIsoPt[tg].Fill(electron.pt, electron.miniPFRelIso_all)
                    # if electron.genPartFlav == 1:
                    #    self.h_electronPt['prompt'].Fill(electron.pt)
                    # if electron.genPartFlav == 5:
                    #    self.h_electronPt['non-prompt'].Fill(electron.pt)
                for nj, jet in enumerate(jets):
                    if nj not in JetPassIdx: continue
                    for key in self.trigLst:
                        if not key.find("Mu") == -1: continue
